...
import { SelectControl, SelectionControl, SelectOption } from "../../components/molecules/SelectControl"
...
const Tools = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<{ title: string } | null>(null)
  ...
  const onHandleBlockchain = (e: SelectOption, add: boolean) => {
    ...
    if (found.length == 0 && add == true) {
      ...
      let newSelectedBlockchain = selectedBlockchain.filter((item) => item.value != e.value)
      setSelectedBlockchain([...newSelectedBlockchain])
    }
  }
  ...
  const onHandleUseCase = (e: SelectOption, add: boolean) => {
    ...
    if (found.length == 0 && add == true) {
      ...
      let newSelectedUseCase = selectedUseCase.filter((item) => item.value != e.value)
      setSelectedUseCase([...newSelectedUseCase])
    }
  }
    ...
    <section className="mb-20">
      ...
      <div className="flex flex-col md:flex-row w-11/12 sm:w-6/12 mx-auto my-8 bg-[#343038] rounded-lg gap-2 px-2 py-2">
        {data.map((item, index) => (
          <div
            key={index}
            className={`md:basis-1/4 px-4 py-2 rounded-md cursor-pointer text-center ${additionalTabClassName(index, completedTab)}`}
      ...
      <div className={`text-white ${activeTab != 0 ? "hidden" : ""}`}>
        <div className="grid w-11/12 grid-cols-3 gap-1 md:w-6/12 md:grid-cols-6 md:gap-4 mx-auto">
          {categoryData.map((item, index) => (
            <div className={`cursor-pointer py-3 ${item.title == selectedCategory?.title ? "bg-[#7A00FF]" : ""} hover:bg-[#7A00FF] rounded-md`} key={index} onClick={() => {
              setSelectedCategory(item)
            }}>
              <CurrencyDollarIcon className="w-8 h-8 mx-auto" />
              <div className="mt-2 text-white text-sm text-center">{item.title}</div>
            </div>
          ))}
        </div>
      </div>
      <div className={`text-white ${activeTab != 1 ? "hidden" : ""}`}>
        <div className="flex flex-1 justify-center py-8 ">
          <div className="relative w-11/12 md:w-5/12">
            <SelectControl data={useCaseItems} onChange={(e) => { onHandleUseCase(e, true) }} value={{ name: "What would you like to work on?", value: "0" }} />
          </div>
        </div>
        <div className="grid w-11/12 grid-cols-2 md:w-8/12 md:grid-cols-3 gap-1 mx-auto">
          <SelectionControl data={selectedUseCase} onChange={(e) => { onHandleUseCase(e, false) }} />
        </div>
      </div>
      <div className={`text-white ${activeTab != 2 ? "hidden" : ""}`}>
        <div className="flex flex-1 justify-center py-8 ">
          <div className="relative w-11/12 md:w-5/12">
            <SelectControl data={blockchainItem} onChange={(e) => { onHandleBlockchain(e, true) }} value={{ name: "Choose your blockchain(s)?", value: "0" }} />
          </div>
        </div>
        <div className="grid w-11/12 grid-cols-2 md:w-8/12 md:grid-cols-3 gap-1 mx-auto">
          <SelectionControl data={selectedBlockchain} onChange={(e) => { onHandleBlockchain(e, false) }} />
        </div>
      </div>
